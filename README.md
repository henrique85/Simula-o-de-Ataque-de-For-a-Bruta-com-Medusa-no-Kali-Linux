Simulação de Ataque de Força Bruta, utilizando a ferramenta Medusa no Kali Linux.

### Requisitos

- Software de Máquina Virtual (no caso será utilizado o VirtualBox).
- VM Kali Linux (link para download aqui https://www.kali.org/get-kali/#kali-virtual-machines).
- VM Metasploitable (link para download [aqui]([https://endereco-do-link.com](https://sourceforge.net/projects/metasploitable/files/))

### Procedimentos iniciais

- Criar um diretório:<br>
mkdir /home/<seu usuário>/script
